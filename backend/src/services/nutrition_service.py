...
class NutritionService:
    ...
    """       
    @staticmethod
    def get_water_recommendation(age_days: int, gender: str) -> str:
        row = NutritionService.get_rien_row(age_days, gender)
        print("DEBUG fila seleccionada:", row)
        if row is not None:
            # Toma AI.11 y AI.12 directamente
            litros = str(row.get("AI.11", "")).strip()
            tazas = str(row.get("AI.12", "")).strip()
            print("DEBUG agua fila:", litros, tazas)
            if litros or tazas:
                return f"Ingesta adecuada de agua: {litros} Litros al día o aproximadamente {tazas} tazas al día"
        return ""
    """
    ...
    def generate_recommendations(assessment: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        """
        agua = NutritionService.get_water_recommendation(assessment.get("age_days", 0), assessment.get("gender", ""))
        print("DEBUG agua:", agua)
        if agua:
            recommendations.append(agua)
    ...
    def export_report_pdf(filename: str, assessment: dict, age_days: int, gender: str, recommendations: dict, child_data: dict):
        ...
        # Observaciones
        obs = child_data.get("nutricionist_observation", "Sin observación del nutricionista.")
    ...
    def get_nutrient_food_table_data(age_days: int, gender: str, weight: Optional[float] = None, kcal_per_day: Optional[float] = None):
        ...
        if kcal_per_day is None:
            raise ValueError("kcal_per_day no puede ser None. Revisa el cálculo de requerimientos energéticos.")
                    ...
                    if row is not None:
                        try:
                            ...
                            if col_idx is not None:
                                print(f"RIEN: df_l.columns = {df_l.columns.tolist()}")
                                print(f"RIEN: Extrayendo valor de columna '{df_l.columns[col_idx]}' para género '{gender}'")
